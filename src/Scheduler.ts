...
export class Scheduler {
  ...
  private convertScheduleSpecificDateToInterval(
    schedule: ScheduleSpecificDate
  ): Interval {
    const interval: Interval = { from: null, to: null };
    const fromDateTime = `${(<Moment>schedule.date).format("YYYY-MM-DD")} ${(<
      Moment
    >schedule.from).format("HH:mm")}`;
    const toDateTime = `${(<Moment>schedule.date).format("YYYY-MM-DD")} ${(<
      Moment
    >schedule.to).format("HH:mm")}`;
    interval.from = moment(fromDateTime, "YYYY-MM-DD HH:mm");
    interval.to = moment(toDateTime, "YYYY-MM-DD HH:mm");
    return interval;
  }

  protected validateAndCastScheduleSpecificDate(
    schedule: ScheduleSpecificDate,
    propertyName: string
  ): ScheduleSpecificDate {
    ...
    if (!(<Moment>s.date).isValid()) {
      throw new Error(
        `${propertyName} "date" must be a date in the format YYYY-MM-DD`
      );
    }

    if (!(<Moment>s.from).isValid()) {
      throw new Error(
        `${propertyName} "from" must be a time in the format HH:mm`
      );
    } else if (!(<Moment>s.to).isValid()) {
      throw new Error(
        `${propertyName} "to" must be a time in the format HH:mm`
      );
  ...
  protected validateAndCastParams(params: AvailabilityParams): void {
    ...
    for (const dayName of this.daysOfWeek) {
      if ((<any>p.schedule)[dayName] !== undefined) {
        try {
          (<any>p.schedule)[dayName] = this.validateAndCastDaySchedule(
            (<any>p.schedule)[dayName]
          );
        } catch (err) {
    ...
    if (p.schedule.weekdays) {
      try {
        p.schedule.weekdays = this.validateAndCastDaySchedule(
          p.schedule.weekdays
        );
      } catch (err) {
    ...
    if (p.schedule.unavailability) {
      for (let i = 0; i < p.schedule.unavailability.length; i += 1) {
        ...
        if ((<ScheduleSpecificDate>unavailability).date !== undefined) {
          const s = this.validateAndCastScheduleSpecificDate(
            <ScheduleSpecificDate>unavailability,
            "unavailability"
          );
          ...
          if (!interval.from.isValid()) {
            throw new Error(
              `unavailability "from" must be a date in the format YYYY-MM-DD HH:mm`
            );
          } else if (!interval.to.isValid()) {
            throw new Error(
              `unavailability "to" must be a date in the format YYYY-MM-DD HH:mm`
            );
    ...
    if (p.schedule.allocated) {
      ...
      for (const allocated of p.schedule.allocated) {
        m = moment(allocated.from, "YYYY-MM-DD HH:mm");
        if (!m.isValid()) {
          throw new Error(
            `"allocated.from" must be a date in the format YYYY-MM-DD HH:mm`
          );
        }
        ...
        const allocatedMinutes =
          p.interval * Math.ceil(allocated.duration / p.interval);
        allocated.to = m.clone().add({ minutes: allocatedMinutes });
      }
    }

    if (p.schedule.custom_schedule) {
      p.schedule.custom_schedule.forEach((customSchedule, key) => {
        p.schedule.custom_schedule[
          key
        ] = this.validateAndCastScheduleSpecificDate(
          customSchedule,
          "custom_schedule"
        );
      });
  ...
  public isTimeslotAvailable(
    ...
    for (const allocated of allocateds) {
      if (
        this.isTimeSameOrAfter(timeSlotStart, allocated.from) &&
        this.isTimeBefore(timeSlotStart, allocated.to)
      ) {
        return false;
      } else if (
        this.isTimeBefore(allocated.from, timeSlotEnd) &&
        this.isTimeAfter(allocated.to, timeSlotStart)
      ) {
        return false;
      }
    }
  ...
  public isDateTimeslotAvailable(
    dateTimeSlotStart: moment.Moment,
    dateTimeSlotEnd: moment.Moment,
    allocateds: any[]
  ): boolean {
    for (const allocated of allocateds) {
      if (
        this.isDateTimeSameOrAfter(dateTimeSlotStart, allocated.from) &&
        this.isDateTimeBefore(dateTimeSlotStart, allocated.to)
      ) {
        return false;
      } else if (
        this.isDateTimeBefore(allocated.from, dateTimeSlotEnd) &&
        this.isDateTimeAfter(allocated.to, dateTimeSlotStart)
      ) {
        return false;
      }
    }
  ...
  public getScheduleForDay(day: moment.Moment): Schedule {
    ...
    if ((<any>this.params.schedule)[weekdayName] !== undefined) {
      if (customSchedule !== undefined) {
        customSchedule.unavailability =
          (<any>this.params.schedule)[weekdayName].unavailability || [];
      ...
      if (customSchedule !== undefined) {
        customSchedule.unavailability =
          (<any>this.params.schedule).weekdays.unavailability || [];
        return customSchedule;
      }
  ...
  public getAvailability(p: AvailabilityParams): Availability {
    ...
    while (curDate.isBefore(this.params.to)) {
      ...
      if (daySchedule !== undefined) {
        ...
        const timeSlotStart = (<moment.Moment>daySchedule.from)
          .clone()
          .year(curDate.year())
          .dayOfYear(curDate.dayOfYear());

        // Loop from <curTime> to <endTime> in <interval> increments
        while (
          this.isTimeBefore(timeSlotStart, <moment.Moment>daySchedule.to)
        ) {
          const timeSlotEnd = timeSlotStart
            .clone()
            .add({ minutes: this.params.duration });
          ...
          if (this.params.schedule.unavailability !== undefined) {
            isAvailable = this.isDateTimeslotAvailable(
              timeSlotStart,
              timeSlotEnd,
              this.params.schedule.unavailability
            );
          }
          ...
          if (isAvailable && this.params.schedule.allocated) {
            let m: Moment;
            const allocatedToday = this.params.schedule.allocated.filter(
              (a: any) => {
                if (!isFunction(a.from)) {
                  m = moment(a.from, "YYYY-MM-DD HH:mm");
                  a.from = m;
                }
                return (
                  m.year() === timeSlotStart.year() &&
                  m.dayOfYear() === timeSlotStart.dayOfYear()
                );
              }
            );
            isAvailable = this.isTimeslotAvailable(
  ...
  public getIntersection(p: IntersectParams): Availability {
    const params: AvailabilityParams = <AvailabilityParams>(
      Object.assign({ schedule: null }, cloneDeep(omit(p, ["schedules"])))
    );
    ...
    for (const day of Object.keys(availabilities[0])) {
      availabilities[0][day] = availabilities[0][day].map(
        (timeAv: TimeAvailability, idx: number) => {
          if (timeAv.available) {
            timeAv.available =
              availabilities.length ===
              availabilities.filter((days: any) => {
                ...
                return (
                  1 ===
                  days[day].filter((time: TimeAvailability) => {
